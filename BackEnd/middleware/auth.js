...
export const authenticateToken = (req, res, next) => {
  ...
  jwt.verify(token, process.env.JWT_SECRET, (err, user) => {
    if (err) {
      ...
      console.warn(
        `⚠ Invalid token for ${req.method} ${req.path}:`,
        err.message
      );
    ...
    req.user = user;
    console.log(
      `✓ Authenticated user: ${user.email || user.userId} for ${req.method} ${
        req.path
      }`
    );
    next();
...
export const authenticateUser = async (req, res, next) => {
  try {
    ...
    req.user = decoded;
    console.log(
      `✓ Authenticated user: ${decoded.email || decoded.userId} for ${
        req.method
      } ${req.path}`
    );
    next();
...
export const requireAdmin = (req, res, next) => {
  ...
  if (req.user.role !== "admin") {
    console.warn(
      `⚠ Access denied: User ${req.user.email || req.user.userId} is not admin`
    );
    return res.status(403).json({
